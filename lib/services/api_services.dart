...
class APIServices {
  static Future<void> loginServerCall({
    ...
    try {
      ...
      log('Request: $requestData'); // Print the request data
      final Response response = await Dio()
          .post('${Urls.baseUrl}${Urls.loginServerCall}', data: requestData);
      log('Response: ${response.data}');
      final responseData = json.decode(response.data);
      if (responseData['errorCode'] == '0000') {
        SharedPrefUtils.saveStr('userId', responseData['userId']);
        // nextPagewithReplacement(context, const BottomNav());
        showCustomToast(
          context: context,
        ...
        showCustomToast(
            context: context, message: responseData['errorMessage']);
      }
    } catch (e) {
  ...
  static dynamic makeApiCall(url, obj) async {
    try {
      final response = await Dio().post('${Urls.baseUrl}/${url}', data: obj);
  ...
  static void createFirebaseToken(userId) async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized ||
      ...
      dynamic result = await makeApiCall(
          "saveFcmToken.php", {"userId": userId, "token": fcmToken});

      log(result['errorMessage']);
    } else {
      log('User declined or has not yet granted permission for notifications');
    }
    // saveFcmToken(fcmToken);
  }
